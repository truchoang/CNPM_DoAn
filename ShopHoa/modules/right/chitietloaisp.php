...
if (isset($_SESSION['up']) && $_SESSION['up'] == 1) {
			$sql = "SELECT * FROM sanpham WHERE MaNhom = $_GET[id] and SoLuong >=  0 ORDER BY DonGia ASC";
			$query = mysqli_query($conn,$sql);
		}
		elseif (isset($_SESSION['down']) && $_SESSION['down'] == 1){
			$sql = "SELECT * FROM sanpham WHERE MaNhom = $_GET[id] and SoLuong >=  0 ORDER BY DonGia DESC";
			$query = mysqli_query($conn,$sql);
		}
		else{
			$sql = "SELECT * FROM sanpham WHERE MaNhom = $_GET[id] and SoLuong >=  0";
			$query = mysqli_query($conn,$sql);
		}
?>
<?php
...
<p align="center" style = "color: coral; font-size:30px; margin: 15px; padding-top: 15px;"> <?php echo $row['TenNhom']?> </p>
<div class= "alllist">
		<ul>
			<?php
